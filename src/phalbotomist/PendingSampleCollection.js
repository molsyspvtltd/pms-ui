...
import {doAssignSampleResult, doGetPendingCollectionRequests} from "./SampleDispatcher";
...
function PendingSampleCollection() {
    ...
    async function onAssignToMeClicked(index){
        const pendingRequest = requests[index]
            ...
            await confirmMessageService.show("Do you want to Assign yourself for sample collecting " + pendingRequest.name + "?")

            doAssignSampleResult(pendingRequest.requestId).subscribe((res)=>{
                appNotification.showSuccess("Request Assigned to you, Please Goto "+ pendingRequest.address+" and  update the details.")

                history.push("/update-sample-report/" + pendingRequest.requestId)
    ...
    function reloadTests() {
        ...
        doGetPendingCollectionRequests()
            .subscribe((pendingTests) => {
                console.log("pending Smaple Collections",pendingTests)
